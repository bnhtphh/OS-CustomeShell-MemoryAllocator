...
freeproc(struct proc *p)
{
  ...
  // Clean up mmap pages if they exist
  if(p->has_mmap) {
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  if(p->has_mmap) {
    ...
    // Map each page from parent to child
    uint64 va = TRAPFRAME - PGSIZE;
    while(va > p->mmap) {
      uint64 pa = walkaddr(p->pagetable, va);
      if(pa != 0) {
...
// Wait for a child process to exit and return its pid.
// Add return status and count 
// Return -1 if this process has no children.
int
kwait2(uint64 status, uint64 count)
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          //Lab04 - is count -> copy out
          if(count != 0 && copyout(p->pagetable, count, (char *)&pp->countp,
                                 sizeof(pp->countp)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    //for loop outsite 
      ...
      acquire(&p->lock);
      if(p->state == RUNNABLE && p->priority >= current) {
